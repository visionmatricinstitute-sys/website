...
- **Phase 7 — Quiz/assignment edit-delete + audit log foundation: shipped**, `feature/quiz-assignment-admin` branch, migration `014_audit_log.sql` (new `audit_log` table + RLS, confirmed absent before this) + `lib/audit-log.ts` (a `logAudit()` helper that never throws — a logging failure can't break the action it's recording) wired into every quiz/assignment admin action, plus: quiz rename + delete (title-only edit — editing individual questions after creation is intentionally not included in this pass, flagged rather than silently skipped, since it needs its own prefilled question-editor UI, a materially bigger build than a rename form), and assignment edit (title/description/due date) + delete (cascades to its submissions — same explicit-admin-action pattern as every other delete added this session). Awaiting Founder to run the migration and merge. Now moving to Phase 8: retrofit `logAudit()` into the delete/create actions from Phases 1-6 once those PRs merge — tracked as a fast-follow, not a new phase to build blind against unmerged branches.
