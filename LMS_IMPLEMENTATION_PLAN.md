...
- **Phase 5 — Student profile page: shipped**, `feature/student-profile` branch, no migration needed (the `profiles` "editable by owner" policy already covered this). `/dashboard/profile` — edit name/phone, change password.
...
- **Re-prioritizing what's left:** instructor role (original phase 7) stays low-priority — there's still only one course/one course-owner, so there's no real user of a second role yet. Audit log (phase 8) is more useful bundled into new admin actions going forward than retrofitted across 6 already-shipped, unmerged branches right now — doing that retrofit once they've merged is a cleaner fast-follow than rebasing 6 branches today.
- **Phase 7 — Quiz/assignment edit-delete + audit log foundation: shipped**, `feature/quiz-assignment-admin` branch, migration `014_audit_log.sql` (new `audit_log` table + RLS, confirmed absent before this) + `lib/audit-log.ts` (a `logAudit()` helper that never throws — a logging failure can't break the action it's recording) wired into every quiz/assignment admin action, plus: quiz rename + delete, and assignment edit (title/description/due date) + delete (cascades to its submissions — same explicit-admin-action pattern as every other delete added this session). Awaiting Founder to run the migration and merge.
- **Phase 8 — Per-question quiz editing + admin search/filters: shipped**, additional commits on `feature/quiz-assignment-admin` (new `/admin/quizzes/[id]` editor — full question/option add/edit/remove, not just the title-only rename phase 7 shipped with; questions are replaced wholesale on save rather than diffed, which is simpler and safe since `quiz_attempts` only references `quiz_id`, not individual question rows, so past attempt history is unaffected) and on `feature/course-authoring` (search on `/admin/courses`). Also added search to the quizzes and assignments admin lists. The `logAudit()` retrofit into Phases 1-6's actions is still a tracked fast-follow once those branches merge, not done here.
- **Phase 9 — Instructor role: shipped**, `feature/instructor-role` branch (off `main`, independent of the other 7 open branches), migration `015_instructor_role.sql`: widens `profiles.role` to include `'instructor'` (via a dynamic constraint lookup, not a hardcoded constraint name, since Postgres auto-named the original one), a new `course_instructors` join table, and a full set of instructor-scoped RLS policies added *alongside* every existing admin policy across courses/modules/chapters/resources/quizzes/quiz_questions/assignments/assignment_submissions — an instructor can only read/write content for courses they're explicitly assigned to; RLS is the real enforcement, not the UI. Also fills two pre-existing gaps found along the way: `module_progress` and `enrollments` had no admin-wide read policy at all before this (only `chapter_progress`/`quiz_attempts` did). Application side: `/admin/instructors` (admin-only — promote a student to instructor, assign/unassign them to courses), `app/admin/layout.tsx` now admits `instructor` alongside `admin`, `DashboardShell` shows a role-appropriate nav subset (instructors see Modules/Assignments/Quizzes but not Live Classes or Instructors — those stay admin-only, both via nav visibility and a page-level redirect, not just RLS), and the modules/quizzes/assignments admin list pages now scope their queries to an instructor's assigned courses via a new `lib/instructor-scope.ts` helper (a UX improvement — RLS already prevented writes to courses they don't own regardless). This branch necessarily copies `lib/audit-log.ts` and `lib/admin-students.ts` from the quiz-assignment-admin/student-admin branches (both genuinely needed here too) — identical content, so no real conflict expected when these branches eventually merge in any order. Awaiting Founder to run the migration and merge. This closes out every item from the original master-prompt gap list.
